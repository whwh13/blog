...
qb_username="whwh" # 改：该为你的qb登录用户名
qb_password="whwh123@wuhao" # 改：改为你qb登录的密码
qb_web_url="https://www.wuhao.work:47832" # 查：改为qb的登录地址，一般可以不改
log_dir="/home/whwh/data-autoqb/qbauto_logs" # 改：改为你日志运行的路径
rclone_dest="google:" # 运行rclone config查看name字段即可；格式就是"XX:"
from_dc_tag="/video/Download" # 改：上传后的相对根目录，可为空
...
function qb_get_status(){
	...
	then
		...
		do
			curtag=$(curl -s "${qb_web_url}/api/v2/torrents/info?filter=completed" --cookie "${cookie}" | jq ".[$i] | .tags" | sed s/\"//g | grep -P "${unfinished_tag}|${ploading_tag}|${finished_tag}|${noupload_tag}")
			echo "*${curtag}*"	
			if [ -z "${curtag}" ]
			then
				curtag="${unfinished_tag}"
			fi
			echo "*${curtag}*"
